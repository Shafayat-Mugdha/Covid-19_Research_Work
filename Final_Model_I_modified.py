...
#data feeding
df = pd.read_csv("Covid_India_I.csv", header = 0) # dataset
df.head()
...
def seird_model(init_vals, params, t):
    ...
    DATE_0, S_0, U_0, Du_0, Ru_0, E_0, Is_0, I_0,  Di_0, Ri_0 = init_vals  # jehetu ajk r real data niye kaj kortesi tai date takeo mention korte hobe.
    DATE, S, U, Du, Ru, E, Is, I, Di, Ri = [DATE_0] , [S_0] , [U_0] , [Du_0], [Ru_0] , [E_0] , [Is_0] , [I_0] ,  [Di_0] , [Ri_0]
    
    beta, sigma, tao, alpha, rho, gamma, delta, miu = params
    ...
    for tic in t[1:]:                # 1st din er initial value deya ache tai 1 theke shuru kore baki shob add dilam.
        ...
        next_E = E[-1] + ( alpha * (S[-1]  * E[-1] / N ) ) - ( rho * Is[-1] ) - ( gamma * I[-1] ) # Exposed
    ...
    return np.stack([DATE, S, U, Du, Ru, E, Is, I, Di, Ri]).T
...
# for i in range(len(uncovered_train)): 
    # print(i,uncovered_train[i])
...
for sample in range(len(uncovered_train)):
    ...
    s_0 = N - du_0 - ru_0 - is_0 - di_0 - ri_0 
    
    beta =   (( u_diff + du_diff + ru_diff ) / (s_0 * u_0 )) * N     # beta
    sigma = ( du_diff / u_0 )     # sigma
    tao = ( ru_diff / u_0 )      # tao
    alpha = (( e_diff + is_diff + i_diff + di_diff + ri_diff) / (s_0 * e_0 ) )* N  # alpha
    ...
    init_vals = str_dates[sample], s_0, u_0, du_0, ru_0, e_0, is_0, i_0, di_0, ri_0
    params = beta, sigma, tao, alpha, rho, gamma, delta, miu
